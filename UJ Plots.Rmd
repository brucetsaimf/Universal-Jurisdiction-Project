...
def_table = states_data %>% 
  ...
  summarise(`First Case` = min(`Case.Year`),
            ...
            `Cases 1950 - 2000` = length(Case.Year[Case.Year <= 2000]),
            `Cases 2000 - Present` = length(Case.Year[Case.Year > 2000]),
            `First Trial` = min(`TrialYear`, na.rm=T),
            `Total Trials` = sum(!(is.na(TrialYear))),
            `Trials 1950 - 2000` = length(Case.Year[Case.Year <= 2000 & !(is.na(states_data$TrialYear))]),
            `Trials 2000 - Present` = length(Case.Year[Case.Year > 2000 & !(is.na(states_data$TrialYear))])) %>%
  mutate(`First Trial` = ifelse(is.infinite(`First Trial`), NA, `First Trial`))

def_table = arrange(def_table, desc(`Total Cases`))
def_table
write.csv(def_table,"Defending States Table")
...
pro_table
write.csv(pro_table,"Prosecuting States Table")
```
